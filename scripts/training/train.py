...
def main():
    ...
    for run_idx, seed in enumerate(seeds[:num_runs]):
        ...
        # --- Aggregation for multi-run ---
        full_eval_results = {
            'XGBoost Base': eval_xgb_base,
            'XGBoost + LLM Notes': eval_xgb_notes,
            'LightGBM Base': eval_lgb_base,
            'LightGBM + LLM Notes': eval_lgb_notes,
            'LSTM Base': eval_base,
            ...
            'LSTM Base (Only Vitals)': eval_only_vitals,
            'LSTM Base (Only Static & Codes)': eval_only_static
        }
        for m_name, res in full_eval_results.items():
            if m_name not in multi_run_results:
                multi_run_results[m_name] = []
            multi_run_results[m_name].append(res)
            
        all_dca_results.append(dca_results)
        all_robustness_results.append(robustness_results)


    # ── Helper to aggregate metrics across runs ──────────────────────────────
    def _m_mean_std(model_name, key='metrics_at_best_t'):
        runs = multi_run_results[model_name]
        metrics = {}
        for metric_name in runs[0][key].keys():
            if metric_name == 'confusion_matrix': continue
            vals = [r[key][metric_name] for r in runs]
            metrics[metric_name] = (np.mean(vals), np.std(vals))
        return metrics

    def _format_metric(m_dict, metric_name, fmt='{mean:.4f}±{std:.4f}'):
        mean, std = m_dict[metric_name]
        return fmt.format(mean=mean, std=std)

    def _note_row_agg(name):
        mb = _m_mean_std(name, 'metrics_at_best_t')
        return (
            f"| {name} | {_format_metric(mb, 'threshold', '{mean:.3f}±{std:.3f}')} | "
            f"{_format_metric(mb, 'roc_auc')} | {_format_metric(mb, 'pr_auc')} | "
            f"{_format_metric(mb, 'precision')} | {_format_metric(mb, 'recall')} | "
            f"{_format_metric(mb, 'f1')} | {_format_metric(mb, 'accuracy')} | {_format_metric(mb, 'brier')} |"
        )

    def _note_row_05_agg(name):
        m5 = _m_mean_std(name, 'metrics_at_0.5')
        return (
            f"| {name} | 0.500 | "
            f"{_format_metric(m5, 'roc_auc')} | {_format_metric(m5, 'pr_auc')} | "
            f"{_format_metric(m5, 'precision')} | {_format_metric(m5, 'recall')} | "
            f"{_format_metric(m5, 'f1')} | {_format_metric(m5, 'accuracy')} | {_format_metric(m5, 'brier')} |"
        )
        
    def _dca_agg(model_name, pt_idx):
        vals = [r[model_name][pt_idx] for r in all_dca_results]
        return f"{np.mean(vals):.4f}±{np.std(vals):.4f}"

    def _robust_agg(dr_idx):
        runs = [r[dr_idx][1] for r in all_robustness_results]
        metrics = {}
        for k in runs[0]['metrics_at_best_t'].keys():
            if k == 'confusion_matrix': continue
            vals = [r['metrics_at_best_t'][k] for r in runs]
            metrics[k] = (np.mean(vals), np.std(vals))
        return metrics


    logging.info("\n================ FINAL ABLATION RESULTS (Test Set) ================")
    logging.info(f"---- @ Best F1 Threshold (Mean ± Std over {num_runs} runs) ----")
    for name in multi_run_results.keys():
        mb = _m_mean_std(name, 'metrics_at_best_t')
        logging.info(
            f"  {name:30s}  t={_format_metric(mb, 'threshold', '{mean:.3f}±{std:.3f}')}  "
            f"AUROC={_format_metric(mb, 'roc_auc')}  PRAUC={_format_metric(mb, 'pr_auc')}  "
            f"F1={_format_metric(mb, 'f1')}"
        )
    logging.info("========================================================\n")
...
| Exp Dir   | `{exp_dir}` |
| **Num Runs** | **{num_runs} (Multi-Run)** |

## Dataset
| Item | Value |
|------|-------|
| Total samples  | {len(Y)} |
| Train / Val / Test | {int(len(Y)*0.7)} / {int(len(Y)*0.1)} / {int(len(Y)*0.2)} |
...
## Ablation Study Results — Test Set @ Best F1 Threshold (Mean ± Std over {num_runs} runs)
| Model | Threshold | AUROC | PRAUC | Precision | Recall | F1 | Acc | Brier |
|-------|-----------|-------|-------|-----------|--------|----|-----|-------|
{_note_row_agg('XGBoost Base')}
{_note_row_agg('XGBoost + LLM Notes')}
{_note_row_agg('LightGBM Base')}
{_note_row_agg('LightGBM + LLM Notes')}
{_note_row_agg('LSTM Base')}
...
{_note_row_agg('LSTM Base (Only Vitals)')}
{_note_row_agg('LSTM Base (Only Static & Codes)')}

## Ablation Study Results — Test Set @ Fixed Threshold 0.5 (Mean ± Std over {num_runs} runs)
| Model | Threshold | AUROC | PRAUC | Precision | Recall | F1 | Acc | Brier |
|-------|-----------|-------|-------|-----------|--------|----|-----|-------|
{_note_row_05_agg('XGBoost Base')}
{_note_row_05_agg('XGBoost + LLM Notes')}
{_note_row_05_agg('LightGBM Base')}
{_note_row_05_agg('LightGBM + LLM Notes')}
{_note_row_05_agg('LSTM Base')}
...
{_note_row_05_agg('LSTM Base (Only Vitals)')}
...
|-----------|-----------|-------|-------|-----------|--------|----|
| 10% | {_format_metric(_robust_agg(0), 'threshold', '{mean:.3f}±{std:.3f}')} | {_format_metric(_robust_agg(0), 'roc_auc')} | {_format_metric(_robust_agg(0), 'pr_auc')} | {_format_metric(_robust_agg(0), 'precision')} | {_format_metric(_robust_agg(0), 'recall')} | {_format_metric(_robust_agg(0), 'f1')} |
| 20% | {_format_metric(_robust_agg(1), 'threshold', '{mean:.3f}±{std:.3f}')} | {_format_metric(_robust_agg(1), 'roc_auc')} | {_format_metric(_robust_agg(1), 'pr_auc')} | {_format_metric(_robust_agg(1), 'precision')} | {_format_metric(_robust_agg(1), 'recall')} | {_format_metric(_robust_agg(1), 'f1')} |
| 30% | {_format_metric(_robust_agg(2), 'threshold', '{mean:.3f}±{std:.3f}')} | {_format_metric(_robust_agg(2), 'roc_auc')} | {_format_metric(_robust_agg(2), 'pr_auc')} | {_format_metric(_robust_agg(2), 'precision')} | {_format_metric(_robust_agg(2), 'recall')} | {_format_metric(_robust_agg(2), 'f1')} |
