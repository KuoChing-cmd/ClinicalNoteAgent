...
class LightGBMRiskPredictor:
    ...
    def _flatten_features(self, X_seq: np.ndarray, X_static: np.ndarray, X_mh: np.ndarray) -> np.ndarray:
        """
    ...
    def predict_batch(self, 
                      ...
                      X_static: np.ndarray, 
                      X_mh: np.ndarray, 
                      X_note: Optional[np.ndarray] = None) -> np.ndarray:
        """
    ...
    def predict_for_simulation(self, 
                               patients_data: Dict[str, Dict[str, np.ndarray]], 
                               use_notes: bool = False) -> Dict[str, float]:
        """
        ...
        for pid in pids:
            ...
            if use_notes:
                if 'X_note' not in data:
                    raise ValueError(f"use_notes is True but 'X_note' missing for patient {pid}")
